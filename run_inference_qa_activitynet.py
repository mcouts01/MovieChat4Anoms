...
def capture_video(video_path, fragment_video_path, per_video_length, n_stage):
    ...
    video.write_videofile(fragment_video_path, logger=None)
...
class Chat:
    ...
    def answer(self, img_list, input_text, msg, max_new_tokens=300, num_beams=1, min_length=1, top_p=0.9,
        ...
        if current_max_len - max_length > 0:
            tqdm.write('Warning: The number of tokens in current conversation exceeds the max length. '
...
if __name__ =='__main__':
    ...
    json_file = open('../val_q.json', 'r')
    json_file = json.load(json_file)
    json_list = []
    for i in range(len(json_file)):
        json_list.append(json_file[i]['video_name'])
        pass
            
    file_list = glob.glob("../v1-2/val/*")
    for i, file in enumerate(file_list):
        file_list[i] = file.split("/")[-1][2:-4]
        
    # file_list = file_list[:400]
        
    joint_list = common_member(json_list, file_list)
    joint_list = list(joint_list)[:400]
    ...
    for i in tqdm(range(len(joint_list))):
        ...
        video_path = os.path.join(args.video_path, f"v_{video_id}.mp4")
